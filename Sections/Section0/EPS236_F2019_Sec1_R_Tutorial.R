#===============================================================================
#
#         FILE: EPSCI236_F2019_Sec1_R_Tutorial.R
#
#       AUTHOR: Yang Li
# ORGANIZATION: Harvard University
#
#         BUGS:
#        NOTES: Modified from Joshua Benmergui and Tianning Zhao's tutorials for previous years
#  DESCRIPTION: E-PSCI 236 R Tutorial for Fall 2019.
#===============================================================================
...
#    source("~/Desktop/EPS236_F2019_Sec1_R_Tutorial.R") 
# 3: Run automatically with output printed to the terminal
#    Rscript EPS236_F2019_Sec1_R_Tutorial.R
#    or
#    R -f EPS236_F2019_Sec1_R_Tutorial.R
# 4: Run in the terminal with an output file created
#    R CMD BATCH EPS236_F2019_Sec1_R_Tutorial.R
#    produces the output file EPS236_F2019_Sec1_R_Tutorial.Rout 
...
save(kNumeric, file = "kNumeric.rda") # save R data stuctures, compressed
...
kCharacter1 <- "hello"   
nchar(kCharacter1) # number of characters
# Substrings
...
kNULL <- NULL
print(kNULL)
kNULL * 5    # numeric(length = 0)
c(kNULL, 5)  # combine values into a vector or list
...
myVec <- rep(5, length = round(runif(n = 1, min = 7, max = 15))) # runif genrates random deviates, rep replicates the values
# __________
...
# Matrix multiplication: %*% (%*%: matrix multiplication; *: the Hadamard product)
...
t(kMatrix2) %*% kMatrix3
t(kMatrix2) * kMatrix3
solve(t(kMatrix2) %*% kMatrix3) # inverse of a square matrix
...
# A while loop
failcount <- 0
tick <- 1       #!!! indices in R start with 1 
while(failcount < 2) { 
...
legend("bottomright", legend = c("Data", "Nothing"), col = c("Red", "blue"),
  lty = c(1, NA), pch = c(NA, 16), lwd = c(2, NA), text.font=4)
...
myLm <- lm(yVals ~ xVals, data = myDF) # linear regression
summary(myLm)
...
# install.packages("maps") # !!! install it first then comment out
...
colcut <- rev(brewer.pal(n = 10, name = "Spectral"))[
  cut(z, breaks = seq(from = 0, to = 100, length = 10))] # divides the range of z into intervals
...
hour(ymd_hms("1990-05-06 12:56:34 UTC"))
year(ymd_hms("1990-05-06 12:56:34 UTC"))
